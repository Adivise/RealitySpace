...
module.exports = async (client, player, track) => {
    try {
        ...
        const now = Date.now();
        const lastStart = player.data.get("lastStart") || 0;
        player.data.set("lastStart", now);

        if (now - lastStart < 2000) {
            const skipCount = (player.data.get("skipCount") || 0) + 1;
            player.data.set("skipCount", skipCount);

            if (skipCount >= 3) {
                player.data.set("skipCount", 0);
        ...
        if (!client.channels.cache.get(player.textId)) return;
        await client.UpdateQueueMsg(player);
    } catch (error) {
        console.log('[playerStart] Error in UpdateQueueMsg:', error.message);
    }

    const db = await GSetup.get(`${player.guildId}_${client.user.id}`);
    if (db && db.setup_enable === true) return;

    const database = await GControl.get(player.guildId);
    if (database === true) return;
